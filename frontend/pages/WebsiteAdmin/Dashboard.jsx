    ...
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        try {
          ...
          // Create mock analytics based on companies data
          const mockAnalytics = {
            totalCompanies: companies.length,
            totalAdmins: companies.reduce((sum, company) => sum + (company.adminsCount || 0), 0),
            newCompaniesThisMonth: Math.floor(companies.length / 2), // Mock value
            pendingApprovals: Math.floor(Math.random() * 5) // Mock value
          };
          
          console.log('Created mock analytics:', mockAnalytics);
  ...
  const fetchCompanies = async () => {
    try {
      ...
      setAnalytics(prev => ({
        ...
        totalAdmins: companiesList.reduce((sum, company) => sum + (company.adminsCount || 0), 0)
      }));
    } catch (error) {
                      ...
                      <table className="data-table" style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0', textAlign: 'left' }}>
                        <thead>
                          <tr style={{ backgroundColor: '#f9fafb' }}>
                            <th style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb', fontWeight: '500', color: '#6b7280' }}>Logo</th>
