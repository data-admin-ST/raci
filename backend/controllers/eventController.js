...
exports.createEvent = async (req, res, next) => {
  try {
    const { name, description, departmentId, employees, tasks } = req.body;
    ...
    if (departmentId) {
      ...
      const result = await db.query(
        `INSERT INTO events (name, description, department_id, hod_id, created_by, document_path)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING event_id, name, description, department_id, hod_id, created_by, document_path, approval_status, created_at`,
        [name, description, departmentId, hodId, createdBy, documentPath]
      );
...
exports.getEvents = async (req, res, next) => {
  try {
    ...
    let query = `
      SELECT e.event_id, e.name, e.department_id, e.approval_status, e.created_at,
...
exports.getEventById = async (req, res, next) => {
  try {
    ...
    const eventResult = await db.query(
      `SELECT e.event_id, e.name, e.description, e.department_id, e.hod_id, 
...
exports.updateEvent = async (req, res, next) => {
  try {
    const eventId = req.params.id;
    const { name, description, departmentId, employees } = req.body;
    ...
    const result = await db.query(
      ...
      [name, description, departmentId, documentPath, eventId]
    );
